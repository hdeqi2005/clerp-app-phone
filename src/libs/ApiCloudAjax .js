...
const isApp = defaultConfig.isRunApp
// 默认配置对象
var options = {
    // 基础url前缀
    baseUrl:'',
    ...
    interceptorResSuc: (res, fn) => {
      ...
      if(res.data.status===10000){
           ...
           window.api.sendEvent({name: 'timeOutLogin'})
           setTimeout(() => {
               window.api.openWin({
                  name: 'login',
                  url: './login.html',
    ...
    // 请求拦截器
    testReq: (config) => {
     // console.warn('=====测试请求拦截器====:'+JSON.stringify(config))
    }
  }
  ...
  class ApiAjax {
     ...
     */
    request (arg) {
      //console.log('=======data post==========:'+JSON.stringify(arg))
     // let _arg = Object.assign(this.getInsideConfig(), arg)
      ...
      return new Promise((res, rej) => {
        // 调用请求拦截器
        let _arg = Object.assign(this.getInsideConfig(), arg)
        //options.testReq(_arg);
       // options.testReq(this.getInsideConfig());
