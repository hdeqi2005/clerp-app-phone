	...
	<div id="app">
		...
		<van-pull-refresh v-model="isLoading" @refresh="searchProdPO">
			<van-panel v-for="(item,index) in purchasesItems" :key="index" class="purchases-panel">
				<div slot="header" class="purchases-header" @click="searchProdPODetail(item.ID1,item.vendName,item.po_SumNoTax,item)">
					<div>
		...
		<van-popup v-model="popupShow" position="left" :style="{ height: '100%',width:'100%'}">
			...
			<van-cell-group>
				...
				<van-cell title="时间:" :value="formatData(currentDetailItem.po_PODate)" />
				<van-cell title="采购单号:" :value="currentDetailItem.po_No" />
				<van-cell title="币别:" value="RMB" />
				<van-cell title="数量:" :value="currentDetailItem.po_Qty" />
