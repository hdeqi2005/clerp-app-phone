<template>
  <div id="app">
        <headerview title='全厂综合查询' @onClickRight="onClickRight"></headerview>
                     ...
                     <!--图表数据展示 -->
                      <van-row  class="body">
                          <compfactoryreportBar style="height:300px;" :value="barData" text="订单面积对照"></compfactoryreportBar>
                      </van-row>
             <van-row  class="body">
   ...
   data(){
    return {
      ...
      dataColumns: [
                      ...
                      {field: 'cac_BoxCOamt', title:'送货金额', width: 136, titleAlign: 'center',columnAlign:'center',isResize:true,
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.cac_BoxCOamt}</span>`
                      },},
                      {field: 'cac_BoxInamt', title:'收款金额', width: 136, titleAlign: 'center',columnAlign:'center',isResize:true,
                      formatter: (rowData, index)=>{
