  ...
  components:{searchForm,deliveryqueryBar},
   data(){
    return {
      ...
      //数据列描述，格式化
      dataColumns: [
                     {field: 'ct_ID', title:'客户编号', width: 136, titleAlign: 'center',columnAlign:'left',
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.ct_ID}</span>`
                      },},
                      {field: 'ct_Name', title:'客户名称', width: 136, titleAlign: 'center',columnAlign:'left',
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.ct_Name}</span>`
                      },},
                       {field: 'ai_Name', title:'地区', width: 136, titleAlign: 'center',columnAlign:'left',
                      formatter: (rowData, index)=>{
