...
export default {
  ...
  data(){
    return {
      ...
      //数据列描述，格式化
      dataColumns: [
                      {field: 'vnct_name', title:'客户名称', width: 136, titleAlign: 'center',columnAlign:'left',
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.vnct_name}</span>`
                      },},
                       {field: 'init_amt', title:'期初金额', width: 136, titleAlign: 'center',columnAlign:'right',
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.amt}</span>`
                      },},
                      {field: 'de_amt', title:'送货金额', width: 136, titleAlign: 'center',columnAlign:'right',
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.amt}</span>`
                      },},
                      {field: 'am_amt', title:'收款金额', width: 136, titleAlign: 'center',columnAlign:'right',
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.amt}</span>`
                      },},
                      {field: 'amt', title:'期末额', width: 136, titleAlign: 'center',columnAlign:'right',
                      formatter: (rowData, index)=>{
                              return `<span">${rowData.amt}</span>`
                      },},
                    ]
